...
var header = Object.keys(json_data[0])

//prepare html html
var headerRowHTML = '<tr>';
for(var i = 0; i<header.length; i++){
    headerRowHTML+='<th>'+header[i]+'</th>';
}
headerRowHTML+='</tr>';


//prepare all the records as html
var allRecordsHTML='';
...
for(var i = 0; i<json_data.length; i++){
    ...
    for(var j = 0;j<header.length;j++){
        var header= header[j];
        allRecordsHTML+= '<td>'+json_data[i][header]+'</td>';
    }
    allRecordsHTML+='</tr>';
}

//Append the table hear and all records
